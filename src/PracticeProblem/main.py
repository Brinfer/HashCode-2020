...
def read_data(file_path: str):
    ...
    with open(file_path, "r") as file:
        g_nb_pizza, g_nb_team_2, g_nb_team_3, g_nb_team_4 = \
...
def select_pizza(p_nb_pizza_to_take):
    ...
    l_nb_test = NB_TEST if len(g_list_pizza) >= NB_TEST//2 \
        else len(g_list_pizza) * 3

    for _ in range(l_nb_test):
        l_list_random_index = random.sample(range(0, len(g_list_pizza)), p_nb_pizza_to_take)
        l_list_pizza_selected_temp = [g_list_pizza[l_list_random_index[0]]]
        l_set_different_ingredient_temp = set(l_list_pizza_selected_temp[0]["ingredient"])
        ...
        for index in l_list_random_index[1:]:
            ...
            l_set_different_ingredient_temp ^= set(
                g_list_pizza[index]["ingredient"])
            l_set_common_ingredient_temp |= set(
                g_list_pizza[index]["ingredient"]) - l_set_different_ingredient_temp

        if (len(l_set_different_ingredient_temp) > len(l_set_different_ingredient_max)) \
            or (len(l_set_different_ingredient_temp) == len(l_set_different_ingredient_max)
...
def main():
    ...
    pr += '\nTime passed: ' + time.strftime("%Hh %Mmin %Ss", time.gmtime(time.time() - chronoStart))
    pr += "\n"
    print(pr)
